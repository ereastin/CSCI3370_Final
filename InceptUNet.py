...
class IRNv4UNet(nn.Module):
  def __init__(self, in_size, Na=1, Nb=1, Nc=1):
    ...
    # down layers
    self.s1 = S1(in_size, base)  # (B, 128, 45, 45) OR (128, 94, 94)
    self.s2 = S2(128, base)  # (B, 640, 22, 22) OR (320, 46, 46)
    self.s3 = S3(320, base)  # (B, 768, 20, 20) (768, 44, 44) OR (384, 44, 44)
    self.s4 = S4(384)  # (B, 1536, 10, 10) (1536, 22, 22) OR (768, 22, 22)
    self.a_n = [A(768, base) for _ in range(Na)]  # (B, 1536, 10, 10) (1536, 22, 22) OR (768, 22, 22)
    self.ra = redA(768)  # (B, 4608, 4, 4) (4608, 10, 10) OR (2304, 10, 10)
    self.b_n = [B(2304) for _ in range(Nb)] # (4608, 10, 10) OR (2304, 10, 10)
    self.rb = redB(2304) # (9216, 4, 4)  consider removing this? OR (4608, 4, 4)
    self.c_n = [C(4608) for _ in range(Nc)] # (4608, 4, 4)
    ...
    self.rev_rb = revRL(4608, 2304)  # (B, 2304, 10, 10)
    self.rev_b = revLB(4608, 2304)  # (B, 2304, 10, 10)
    self.rev_ra = revRL(2304, 768)  # (B, 768, 22, 22)
    self.rev_a = revLA(1536, 768)  # (B, 768, 22, 22)
    self.rev_s4 = revS(768, 384, stride=2, padding=1)  # (B, 384, 44, 44)
    self.rev_s3 = revScat(768, 320, stride=1)  # (B, 320, 46, 46)
    self.rev_s2 = revS(320, 128, stride=2, padding=0)  # (B, 128, 94, 94)
    self.rev_s1 = revScat(256, in_size, stride=1)  # (B, 64, 96, 96)

    # TODO: make this 1x1 kernel!
