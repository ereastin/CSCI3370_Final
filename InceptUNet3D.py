...
class revRL(nn.Module):
  def __init__(self, in_size, out_size, op=1, b=False, drop_p=0.0):
    ...
    half_out = out_size // 2
    self.b1 = nn.Sequential(
        nn.ConvTranspose2d(in_size, in_size // 12, kernel_size=3, stride=2, padding=0, output_padding=op, bias=b), nn.ReLU(),
        nn.ConvTranspose2d(in_size // 12, half_out, kernel_size=3, stride=1, padding=1, bias=b), nn.ReLU(),
        nn.BatchNorm2d(half_out, eps=1e-2)
    )
    self.b2 = nn.Sequential(
        nn.ConvTranspose2d(in_size, in_size // 12, kernel_size=3, stride=1, padding=1, bias=b), nn.ReLU(),
...
class revScat(nn.Module):
  def __init__(self, in_size, out_size, stride=1, p=(1, 0), dd=1, op=0, b=False, ks=(1, 2, 2), kp=(0, 1, 1), d=1):
...
class IRNv4_3DUNet(nn.Module):
  def __init__(self, in_size, depth=42, Na=1, Nb=1, Nc=1, base=32, bias=False, drop_p=0.0, lin_act=0.1):
    ...
    self.s2 = S2(2 * base, base, b=bias)  # -> (B, 160, , H, W)
    self.s3 = S3(5 * base, base, b=bias)  # -> (B, 192, , H, W)
    ...
    self.b_n = [B(36 * base, base, b=bias, lin_act=lin_act) for _ in range(Nb)]  # -> (B, 1152, D, H, W)
    ...
    self.rev_s4 = revS(12 * base, 6 * base, stride=(1, 1), p=1, op=0, b=bias)  # (B, 192, ...)
    self.rev_s3 = revScat(12 * base, 5 * base, stride=(2, 1), p=1, b=bias)  # (B, 160, ...)  CAT
    self.rev_s2 = revS(5 * base, 2 * base, stride=(2, 1), p=(0, 1), op=(1, 0), b=bias)  # (B, 128, ...)
    self.rev_s1 = revScat(4 * base, in_size, stride=(1, 1), p=(0, 1), op=0, b=bias, ks=(1, 1, 4), kp=0, d=(1, 2, 3))  # (B, 5, ...)  CAT

    # TODO: cant quite get to 43 tall from here without some farther up changes or crazy padding..
    self.ff = nn.ConvTranspose2d(in_size, in_size, kernel_size=(8, 3), stride=1, padding=(0, 1))    
    self.final = nn.Conv2d(in_size, 1, kernel_size=3, stride=1, padding=0)
